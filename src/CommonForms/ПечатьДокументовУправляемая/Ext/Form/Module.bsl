...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = УникальныйИдентификатор И Параметр.ИмяОбъектаМетаданныхМакета = НастройкаПечатнойФормы.ПутьКМакету Тогда
		...
		ИначеЕсли ИмяСобытия = "ОтказОтИзмененияМакета" Тогда
...
Процедура Отправить(Команда)
	...
	УправлениеОтчетами.ОтправитьДокументПоЭлектроннойПочте(ЭтаФорма[НастройкаПечатнойФормы.ИмяРеквизита], Заголовок);
...
Процедура СформироватьПечатныеФормы(КоллекцияПечатныхФорм, ИменаМакетов, Отказ)
	...
	Если ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда
		ПечатьВыполнена = УправлениеПечатьюПереопределяемый.ПечатьПоВнешнемуИсточнику(Параметры.ИсточникДанных,
			Параметры.ПараметрыИсточника, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
		Отказ = Отказ Или Не ПечатьВыполнена;
	Иначе
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	ДоступноИзменениеМакетов = Пользователи.РолиДоступны("ИзменениеМакетовПечатныхФорм");
