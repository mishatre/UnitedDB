...
Процедура ВосстановитьНастройкиОбновления(ВыдаватьСообщения = Истина) Экспорт
	...
	ПроксиСерверТребуетАутентификации = ?(ТипЗнч(ПроксиСерверТребуетАутентификации) <> Тип("Булево"), Ложь, ПроксиСерверТребуетАутентификации);
...
Процедура СохранитьНастройкиОбновления() Экспорт
	...
	СохранитьЗначение("ПроксиСерверТребуетАутентификации", ПроксиСерверТребуетАутентификации);
...
Функция КаталогШаблонов()
	...
	КаталогПоУмолчанию = КаталогAppData() + "1C\1Cv82\tmplts\";
	СисИнфо = Новый СистемнаяИнформация;
	...
	Если (Число(ПодстрокиВерсии[0]) = 8 И Число(ПодстрокиВерсии[1]) >= 2) ИЛИ (Число(ПодстрокиВерсии[0]) > 8) Тогда
		...
		Пока Стр <> Неопределено Цикл
			...
			Если ПозицияРазделителя = 0 Тогда
				Продолжить;
			КонецЕсли; 
			ВозвращаемыйКаталог = ДобавитьКонечныйРазделительПути(СокрЛП(Сред(Стр, ПозицияРазделителя + 1)));
			Возврат ?(ФайлСуществует(ВозвращаемыйКаталог), ВозвращаемыйКаталог, КаталогПоУмолчанию);
		КонецЦикла; 
...
Функция ПроверитьДоступКИБ(Знач ВыдаватьСообщения = Истина, ИнформацияОбОшибке = "") Экспорт
...
#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		КонецЕсли; 
#КонецЕсли
		ИнформацияОбОшибке = "Ошибка при проверке подключения к информационной базе." + Символы.ПС + ОписаниеОшибки;
		ЗаписатьОшибку(ИнформацияОбОшибке);
	КонецПопытки;	
	
	Если Результат И НЕ ЭтоФайловаяБаза() Тогда
...
Процедура УточнитьПараметрыПодключенияККластеру(ВыдаватьСообщения = Истина)
	...
	Если НестандартныеПортыСервера Или ЭтоФайловаяБаза() Тогда
		Возврат;
	КонецЕсли;
...
Функция ЭтоФайловаяБаза() Экспорт
	...
	ПозицияПоиска = Найти(Врег(СтрокаСоединенияИнформационнойБазы()), "FILE=");
...
Функция ВставитьПараметрыСкрипта(Знач Текст)
	...
	ВерсияПлатформы = ОпределитьВерсиюПлатформы();
	
	Если ВерсияПлатформы = "8.3" Тогда
		ШиринаОкнаЗаставкиОбновления = 600;
		ВысотаОкнаЗаставкиОбновления = 320;
	ИначеЕсли ВерсияПлатформы = "8.2" Тогда
		ШиринаОкнаЗаставкиОбновления = 400;
		ВысотаОкнаЗаставкиОбновления = 258;
	Иначе
		ШиринаОкнаЗаставкиОбновления = 400;
		ВысотаОкнаЗаставкиОбновления = 240;
	КонецЕсли;
	...
	Результат = СтрЗаменить(Результат, "[ШиринаОкнаЗаставкиОбновления]", ШиринаОкнаЗаставкиОбновления);
	Результат = СтрЗаменить(Результат, "[ВысотаОкнаЗаставкиОбновления]", ВысотаОкнаЗаставкиОбновления);
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим) 
	...
	Если ИнтерактивныйРежим Тогда
		...
		Если ВерсияПлатформы = "8.3" Тогда
			БиблиотекаКартинок.ЗаставкаОбновленияКонфигурации83.Записать(КаталогВременныхФайловОбновления + "splash.png");
		ИначеЕсли ВерсияПлатформы = "8.2" Тогда
			БиблиотекаКартинок.ЗаставкаОбновленияКонфигурации82.Записать(КаталогВременныхФайловОбновления + "splash.png");
		Иначе
			БиблиотекаКартинок.ЗаставкаОбновленияКонфигурации.Записать(КаталогВременныхФайловОбновления + "splash.png");
		КонецЕсли;
