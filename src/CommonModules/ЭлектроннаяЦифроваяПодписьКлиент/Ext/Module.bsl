...
// Открывает персональные настройки ЭЦП.
//
// Параметры:
//   Модально - Булево - Открывать форму поверх всех остальных.
//
Процедура ОткрытьФормуНастройкиЭЦП() Экспорт
...
Функция ПолучитьМассивСтруктурСертификатов(знач ТолькоЛичные) Экспорт
	...
	ТекущаяДата = ОбщегоНазначенияКлиент.ДатаСеанса(); // Используется для выявления истекших сертификатов, которые хранятся на клиентском компьютере.
...
Процедура ОткрытьПодпись(ТекущиеДанные) Экспорт
	...
	Если ОбъектУказан Тогда
		...
		Если СтруктураСертификата <> Неопределено Тогда
			...
			ОткрытьФорму("ОбщаяФорма.ПодписьЭЦП", ПараметрыФормы);
		КонецЕсли;
