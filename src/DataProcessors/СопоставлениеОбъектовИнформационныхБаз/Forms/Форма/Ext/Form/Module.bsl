...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		НаименованиеЭтойПрограммы,
		НаименованиеВторойПрограммы
	);
КонецПроцедуры
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если УстановитьСвязьБыстро() Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция УстановитьСвязьБыстро()
	
	Если Элементы.ТаблицаСопоставления.ВыделенныеСтроки.Количество() = 2 Тогда
		
		ИдентификаторСтроки1 = Элементы.ТаблицаСопоставления.ВыделенныеСтроки[0];
		ИдентификаторСтроки2 = Элементы.ТаблицаСопоставления.ВыделенныеСтроки[1];
		
		Строка1 = ТаблицаСопоставления.НайтиПоИдентификатору(ИдентификаторСтроки1);
		Строка2 = ТаблицаСопоставления.НайтиПоИдентификатору(ИдентификаторСтроки2);
		
		Если    (Строка1.СтатусСопоставления = -1 И Строка2.СтатусСопоставления = +1)
			ИЛИ (Строка1.СтатусСопоставления = +1 И Строка2.СтатусСопоставления = -1) Тогда
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Установить'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Отмена'"));
			
			Ответ = Вопрос(НСтр("ru = 'Установить соответствие между выбранными объектами?'"), Кнопки,, КодВозвратаДиалога.Да);
			
			Если Ответ = КодВозвратаДиалога.Да Тогда
				
				ДобавитьНеутвержденноеСопоставлениеНаКлиенте(ИдентификаторСтроки1, ИдентификаторСтроки2);
