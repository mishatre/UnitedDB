...
Функция ПолучитьДоступныеУчетныеЗаписи(знач ДляОтправки = Неопределено,
										...
										знач ВключатьСистемнуюУчетнуюЗапись = Ложь) Экспорт
...
Функция ОтправитьСообщение(знач УчетнаяЗапись,
	...
	РеквизитыОтправителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, "Логин,АдресЭлектроннойПочты");
	
	// добавляем к письму имя отправителя
	Письмо.ИмяОтправителя              = РеквизитыОтправителя.Логин;
	Письмо.Отправитель.ОтображаемоеИмя = РеквизитыОтправителя.Логин;
...
Функция ЗагрузитьСообщения(знач УчетнаяЗапись,
	...
	Иначе
		НастройкиТранспорта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, "ОставлятьКопииСообщенийНаСервере,КоличествоДнейУдаленияПисемССервера");
		Если НастройкиТранспорта.ОставлятьКопииСообщенийНаСервере Тогда
			
			Если ЗаголовкиИдентификаторы.Количество() =0
			   И НастройкиТранспорта.КоличествоДнейУдаленияПисемССервера > 0 Тогда
				...
				Для Каждого ЭлементЗаголовок Из Заголовки Цикл
					...
					Если РазницаДат >= НастройкиТранспорта.КоличествоДнейУдаленияПисемССервера Тогда
...
Функция СформироватьИнтернетПрофиль(знач УчетнаяЗапись,
	...
	Профиль.Пользователь = НастройкиСоединения.Логин;
	Если НастройкиСоединения.ВремяОжиданияСервера > 0 Тогда
		Профиль.ВремяОжидания = НастройкиСоединения.ВремяОжиданияСервера;
	КонецЕсли;
	...
	Если ФормироватьSMTPПрофиль Тогда
		Профиль.АдресСервераSMTP = НастройкиСоединения.SMTPСервер;
		Профиль.ПортSMTP         = НастройкиСоединения.ПортSMTP;
		
		Если НастройкиСоединения.ТребуетсяSMTPАутентификация Тогда
			Профиль.АутентификацияSMTP = СпособSMTPАутентификации.ПоУмолчанию;
			Профиль.ПользовательSMTP   = НастройкиСоединения.ЛогинSMTP;
	...
	Если ФормироватьPOP3Профиль Тогда
		Профиль.АдресСервераPOP3 = НастройкиСоединения.POP3Сервер;
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.1.0") > 0 Тогда
		Профиль.ИспользоватьSSLPOP3 = НастройкиСоединения.ИспользоватьЗащищенноеСоединениеДляВходящейПочты;
		Профиль.ИспользоватьSSLSMTP = НастройкиСоединения.ИспользоватьЗащищенноеСоединениеДляИсходящейПочты;
	КонецЕсли;
