...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаблицаАвтоматическиСопоставленныхОбъектовПустая = (Объект.ТаблицаАвтоматическиСопоставленныхОбъектов.Количество() = 0);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТаблицаАвтоматическиСопоставленныхОбъектовПустая Тогда
		
		Отказ = Истина;
		
		Предупреждение(НСтр("ru = 'Одинаковые данные не найдены.
			|Попробуйте задать другие условия сопоставления.'")
		);
	КонецЕсли;
...
Процедура УстановитьВидимостьПолейТаблицы(Знач ИмяТаблицыФормы, Знач МаксимальноеКоличествоПользовательскихПолей)
	...
	Для Каждого Элемент ИЗ Объект.СписокИспользуемыхПолей Цикл
		...
		ЭтаФорма.Элементы[ПолеИсточника].Заголовок = Элемент.Представление;
		ЭтаФорма.Элементы[ПолеПриемника].Заголовок = Элемент.Представление;
